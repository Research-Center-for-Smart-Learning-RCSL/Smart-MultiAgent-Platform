...
- **[R24.32]** Chat: single-pane at < 768 px; side panels (agent list, attachments) become a drawer. At >= 1024 px the right rail is a persistent column whose width the user may resize between a documented minimum and maximum, persisted locally per browser and re-clamped so the message column always retains its minimum share. Every rail panel scrolls within the rail: rail content is never clipped without a reachable scroll region.
