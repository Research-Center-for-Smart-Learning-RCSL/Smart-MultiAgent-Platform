...
- **[R12.03a]** The `/workspace/agent-files/` region of an Agent's volume is a **projection** of that Agent's `agent_workspace_files` rows, and `/workspace/skills/` a projection of its bound-skill scripts, not independent state. Each turn that stages either makes the region equal the staged set: a file no longer in the set is removed from the volume, not merely omitted from the Agent's file listing. Reconciliation is authoritative over any cached belief about the volume's contents. The rest of the volume — the `file` tool's own state and per-room session directories — is Agent-authored and is never reconciled against a table.
