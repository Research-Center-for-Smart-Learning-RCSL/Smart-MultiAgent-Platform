...
- **[R24.28]** *(amended 2026-08-21)* Design tokens live in `frontend/src/shared/styles/main.css` in the Tailwind v4 `@theme` block as CSS custom properties, themed light/dark via the `data-theme` attribute on `<html>`. The token vocabulary covers colour (surface roles canvas / raised / recessed, semantic, status tint/on, and two border weights: boundary and interior), spacing, typography (size, weight, line-height and tracking), control height, radius, shadow/elevation, motion (duration/easing/distance), focus, layout, and z-index scales. Components consume tokens and declare no literal type, spacing, elevation or neutral colour value.
