...
_EXPECTED_ALLOW: dict[Capability, set[Role]] = {
    ...
    Capability.KEY_VIEW_USAGE_PROJECT: {Role.ORG_OWNER, Role.ORG_MEMBER, Role.PROJECT_OWNER, Role.PROJECT_MEMBER},
    ...
    Capability.PROJECT_CREATE_UNDER_USER: {Role.ORG_OWNER, Role.ORG_MEMBER, Role.PROJECT_OWNER, Role.PROJECT_MEMBER},
    ...
    Capability.CHAT_SEND: {Role.ORG_OWNER, Role.ORG_MEMBER, Role.PROJECT_OWNER, Role.PROJECT_MEMBER, Role.GUEST},
    Capability.GUEST_LINK_MANAGE: {Role.ORG_OWNER, Role.PROJECT_OWNER},
    Capability.CHAT_EXPORT: {Role.ORG_OWNER, Role.ORG_MEMBER, Role.PROJECT_OWNER, Role.PROJECT_MEMBER, Role.GUEST},
    Capability.MESSAGE_DELETE: {Role.ORG_OWNER, Role.ORG_MEMBER, Role.PROJECT_OWNER, Role.PROJECT_MEMBER, Role.GUEST},
...
async def test_matrix_full_sweep(cap: Capability, role: Role) -> None:
    ...
    resolver = FakeResolver(
        roles=frozenset({role}), oc=True, room_participant=True,
    )
    scope = Scope(
        org_id=uuid.uuid4(),
...
async def test_guest_chat_send_requires_room_participation() -> None:
    ...
    denied = await decide(
        principal, Capability.CHAT_SEND, scope,
