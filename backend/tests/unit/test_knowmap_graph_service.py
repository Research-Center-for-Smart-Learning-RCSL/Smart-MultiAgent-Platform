"""KnowmapGraphService — mirrors GraphRagGraphService's contract (Phase 3β, R11.24, AC-1).

No exemplar test exists for the Concept Map equivalent (graphrag_graph_service.py
ships untested), so this is new coverage rather than a mirrored test — see the
task spec's Test Plan.
"""
...
def _patch_common(monkeypatch: pytest.MonkeyPatch, *, cfg: _FakeCfg | None) -> None:
    monkeypatch.setattr(km_mod, "KnowmapConfigRepository", _FakeRepo(cfg))
    monkeypatch.setattr(km_mod, "Neo4jAsyncDriver", _FakeDriver)
...
async def test_get_graph_assembles_nodes_and_edges_self_consistently(monkeypatch) -> None:
    ...
    service = km_mod.KnowmapGraphService(db=None)  # type: ignore[arg-type]
...
async def test_get_graph_clamps_limit_to_bounds(monkeypatch) -> None:
    ...
    assert _FakeDriver.last_limit == km_mod.MAX_GRAPH_LIMIT
