...
approvals = sa.Table(
    ...
    sa.Column("workflow_run_id", pg.UUID(as_uuid=True),
              ...
              nullable=False),
    sa.Column("mode", pg.ENUM("single", "majority", "consensus",
    ...
    sa.Column("state", pg.ENUM("pending", "approved", "rejected",
                               "timeout_leader",
                               name="approval_state", create_type=False),
              nullable=False, server_default=sa.text("'pending'::approval_state")),
    sa.Column("started_at", sa.TIMESTAMP(timezone=True), nullable=False,
              server_default=sa.text("now()")),
    sa.Column("ended_at", sa.TIMESTAMP(timezone=True), nullable=True),
)
...
instructions = sa.Table(
    ...
    sa.Column("payload", pg.JSONB, nullable=False,
              server_default=sa.text("'{}'::jsonb")),
    sa.Column("state", pg.ENUM("issued", "delivered", "completed",
                               "rejected_loop", "timeout",
                               name="instruction_state", create_type=False),
              nullable=False, server_default=sa.text("'issued'::instruction_state")),
