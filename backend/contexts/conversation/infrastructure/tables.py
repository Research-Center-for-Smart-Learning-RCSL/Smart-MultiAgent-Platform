...
messages = sa.Table(
    ...
    sa.Column("id", pg.UUID(as_uuid=True), primary_key=True,
              server_default=sa.text("gen_random_uuid()")),
    sa.Column("chatroom_id", pg.UUID(as_uuid=True),
              sa.ForeignKey("chatrooms.id", ondelete="CASCADE"), nullable=False),
...
message_attachments = sa.Table(
    ...
    sa.Column("chatroom_id", pg.UUID(as_uuid=True),
              sa.ForeignKey("chatrooms.id", ondelete="CASCADE"), nullable=True),
    sa.Column("uploaded_by_user_id", pg.UUID(as_uuid=True),
