"""Cohere probe — `GET /v1/models` (R7.02)."""
...
async def probe_cohere(secret: str) -> ProbeResult:
    headers = {"authorization": f"Bearer {secret}"}
    try:
        async with new_http_client() as client:
            resp = await client.get(probe_url("cohere_base_url", _PATH), headers=headers)
