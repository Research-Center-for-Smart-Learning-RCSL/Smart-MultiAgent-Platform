...
api_keys = sa.Table(
    ...
    sa.Column("provider",
              pg.ENUM(
                  "claude", "openai", "gemini", "voyage", "cohere",
                  name="api_key_provider", create_type=False,
              ),
              nullable=False),
...
key_projects = sa.Table(
    ...
    sa.Column("key_id", pg.UUID(as_uuid=True),
              sa.ForeignKey("api_keys.id", ondelete="CASCADE"), primary_key=True),
...
key_group_members = sa.Table(
    ...
    sa.Column("group_id", pg.UUID(as_uuid=True),
              sa.ForeignKey("key_groups.id", ondelete="CASCADE"), primary_key=True),
    sa.Column("key_id", pg.UUID(as_uuid=True),
              sa.ForeignKey("api_keys.id", ondelete="CASCADE"), primary_key=True),
    sa.Column("priority", sa.Integer, nullable=False),
    sa.Column("rotate_on_error_codes", pg.ARRAY(sa.Integer), nullable=False,
...
search_keys = sa.Table(
    ...
    sa.Column("project_id", pg.UUID(as_uuid=True),
              sa.ForeignKey("projects.id", ondelete="CASCADE"), nullable=False),
    sa.Column("provider",
              pg.ENUM("brave", "serper", "tavily", "google_cse",
                      name="search_provider", create_type=False),
              nullable=False),
    sa.Column("ciphertext", sa.LargeBinary, nullable=False),
    sa.Column("nonce", sa.LargeBinary, nullable=False),
    sa.Column("dek_wrapped", sa.Text, nullable=False),
