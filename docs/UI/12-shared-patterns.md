...
2. **Server-side**: RFC 7807 top-level `field_errors` extension member mapped to vee-validate via `useServerErrors()`. Only entries with `location: "body"` reach a field; a `query`/`path`/`header`/`cookie` failure falls through to the caller's domain message
...
| `validation-error` | Map the top-level `field_errors` extension member to form fields (`location: "body"` entries only) |
