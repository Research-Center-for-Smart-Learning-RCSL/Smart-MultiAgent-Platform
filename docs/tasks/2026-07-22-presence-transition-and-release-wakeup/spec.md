...
R15.05b roster gate (`backend/contexts/orchestration/application/wakeup_service.py:227-230`)
...
- **Observed** — the roster SET `ws:presence:{room_id}` is written by `join`
  ...
  `_notify_presence(has_live_users=False)` (`chatroom.py:32-43`, `:142`) never runs. **The single
  remaining guard, `wakeup_service.py:227-230`, reads the same unreconciled set and passes.**
...
(`wakeup_service.py:227-230`) does not catch it. Post-C1 (Q-6) step 4 is the whole defect: the gate
...
3. Within the next 300 s (`_SET_TTL_SECONDS`, `presence.py:36`) the 30-second silence sweep
   (`orchestration.py:226-296`, cron at `main.py:320`) evaluates the trigger: the silence timestamp
   is present and the elapsed window has passed (`wakeup_service.py:210-217`), autostop has not
   tripped (`:220-223`), and the roster read at `:228` sees the ghost, so the wake fires. A full
   provider turn runs against an empty room.
...
6. The gate that would otherwise catch the ghost — `wakeup_service.py:227-230`, the unconditional
...
- Ghost-holding roster keys are continuously refreshed, so the room stays visible to the nightly
  reconciler's `scan_iter` (`presence.py:173-177`), which SREMs the ghost from the roster.
...
entirely (`presence.py:190-191`), so the nightly scrub never reaches it. The window in which a ghost
...
30 s sweep (`wakeup_service.py:227-230`). What remains true, and is the entire justification for
...
- `wakeup_service.py:108-112` (`every_n_messages` empty-room gate) — **confirmed affected by the same
...
- `wakeup_service.py:227-230` (silence roster gate) — **confirmed affected**, as analysed in §5
...
(`wakeup_service.py:228`), the per-message every_n gate (`:109`), and an explicit REST snapshot
(`chatrooms.py:648`). (Cron line reference corrected 2026-07-27.)
...
every sweep (`wakeup_service.py:227-230`) and is level-triggered by construction. `set_silence_active`
...
(`wakeup_service.py:196-199,227`, O-2/R28.04) — unchanged by C1 and unchanged here.
...
`test_wakeup_service.py:52-81` (`test_allow_self_open_true_does_not_fire_into_empty_room` and
`test_allow_self_open_false_still_does_not_fire_into_empty_room`) pin that an empty roster suppresses
the trigger on every evaluation, which is what A2 was trying to guarantee. Moved into T-9's
...
at `:92-100` and asserts exactly the surviving half: `is_observer=True` with `room_members=[]` still
...
`test_wakeup_service.py:52-81` (an empty roster suppresses for both `allow_self_open` values — this
is the post-C1 statement of the property A2 was meant to provide) and `:92-100`
...
- [ ] **AC-6**: Observer bindings still bypass the roster re-check (O-2/R28.04), verified by
  `test_wakeup_service.py:92-100` staying green.
