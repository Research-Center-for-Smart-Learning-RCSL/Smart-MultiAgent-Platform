...
| Q-6 | Does this depend on another active dossier? | No; `depends_on: []`. | No active dossier touches identity onboarding, this policy table, Admin users or the planned maintenance commands. The conversation and provider/runtime dossiers are disjoint. |
...
Import legacy state, activate explicitly after old replicas drain, use a short-lived v2 cache in
active state, and freeze/verify before rollback. This is more operationally explicit but every
transition is serializable and testable.
...
State transitions are explicit maintenance operations, not incidental startup side effects.
...
- The active reader uses process cache, v2 Redis and PostgreSQL in that order, subject to Q-8's
  freshness rules. Compatibility uses the atomic legacy reader. Frozen uses PostgreSQL only.
...
- Add an expand-compatible migration after `0083` for a singleton `email_domain_policies` row with
  checked mode/state, normalized `text[]` lists, integer version, nullable
  `legacy_mirrored_version`, timestamps and updating-Admin FK. The migration creates no default.
...
- Active cache key `config:email_domain:policy:v2` contains schema, version, mode and both lists in
  one JSON value written with `EX 30`. Reads obtain value and PTTL together. Only a successful DB
  read may repair/extend it; a Redis-derived snapshot never does.
- Add idempotent `activate-email-domain-policy` and
  ...
  atomically replaces all legacy keys and records the verified version.
...
- [ ] Performance — active readers make at most one singleton DB read per 30-second expiry window;
  no cache entry outlives its Redis PTTL.
...
- [ ] AC-8: a forward/rollback mixed-version test proves compatibility old/new readers enforce one
  ...
  enforce the same verified rollback snapshot.
...
- [ ] AC-10: targeted unit, DB/Redis integration, frontend component, lint, typecheck and build gates
  pass.
- [ ] AC-11: v2 missing, malformed, expired, evicted or unreadable always falls back to DB; when DB
  is also unavailable the request returns typed 503 and never uses `off`.
...
- **Redis integration** — atomic legacy read/replace, key-type errors, v2 GET+PTTL, schema/version
  validation, TTL non-renewal and DB repair.
