...
| Q-6 | Does this depend on another active dossier? | No; `depends_on: []`. | No active dossier touches identity onboarding, admin users, the planned policy table, or the relevant frontend admin surface. The two other 2026-08-30 FU dossiers work in conversation/shared frontend code; their overlap dependency is between those two and does not sequence this work. |
