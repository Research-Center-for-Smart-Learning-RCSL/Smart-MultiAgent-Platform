...
messages. `replayDelta` appends (`frontend/src/slices/conversation/composables/useChatroomSocket.ts:84-105`,
applying each row through `applyMessageCreated` at `:135-149`), and the `since` query it calls
...
- **Observed** — `replayDelta` (`useChatroomSocket.ts:84-105`) is the only reconnect recovery for
  message state, and it appends only (`:96`). The backend `since` window cannot express a deletion
  ...
  `message.updated` handlers (`useChatroomSocket.ts:202-212`, `:188-201`) are live-only.
...
- **Observed** — the connect burst resyncs messages, presence and activation
  (`useChatroomSocket.ts:353-355`) and nothing else. `approval.requested` / `approval.resolved`
  (`:268-291`) are the sole writers into the orchestration store, which is pure in-memory WS state
  (`frontend/src/shared/stores/orchestration.ts:17`). The only REST list is run-scoped
  (`backend/app/api/v1/orchestration.py:229-247`), and the conversation slice never calls it. A
  missed `approval.requested` is unrecoverable, and the later `approval.resolved` is a no-op on an
  unknown id (`orchestration.ts:32-35`), so the gate stays invisible for the rest of the run.
...
- **Expected** — the codebase states the expectation itself: the identical 422 on the `since` cursor
  ...
  (`useChatroomSocket.ts:97-104`). A dead cursor must degrade to a refetch, not to a permanent
  error. Plus `[R24.23]`.

### F-19 — `message.updated` refetches are unsequenced

- **Observed** — `useChatroomSocket.ts:188-201` fires `getMessage(updatedId)` and writes the result
  into the cache on resolve, with no generation counter and no version comparison. Every sibling
  async path in the same file guards: `replayGeneration` (`:67`, captured `:92`, checked `:95`),
  `activationGeneration` (`:68`, `:118-127`), delete tombstones (`:76-82`).
...
- **Observed** — `useChatroomMessages` destructures nothing from `useQuery` beyond the data
  (`:79-86`) and returns no `isLoading`/`isPending`/`isError` (`:327-348`).
...
|---|---|---|---|
| Q-1 | **Must this dossier land before `docs/tasks/2026-07-22-chatroom-socket-lifecycle/` (a2u F-1, the 120s reaping)?** | **UNRESOLVED — the two dossiers reached opposite conclusions independently. See the conflict note immediately below this table; the user must break the tie.** This dossier's own argument, recorded for that decision, is: land this one first. Not a build dependency either way; a precedence constraint on the state of `main` between the two merges. | The audit states it: `findings.md:83-85` and `:355-357` record that F-11 "becomes *worse* once F-1 is fixed". The mechanism, stated precisely: F-1 currently closes every idle socket at 120s and `ws-manager.ts` reconnects at the 1s floor, so the connect burst at `useChatroomSocket.ts:347-356` runs roughly every two minutes for free. That burst does **not** repair deletions, edits or approvals — which is why F-11/F-13 are findings at all — but it does keep presence, activation and the created-message cursor continuously fresh, and it keeps every disconnect sub-second. Fixing F-1 changes the disconnect profile from *frequent, ~1s, benign* to *rare, arbitrarily long, consequential*: a sleeping laptop or a tunnel now produces a single gap of minutes during which an unbounded number of edits, deletions and approvals accumulate, with no periodic anything. The per-incident stakes of exactly this reconciliation gap rise sharply. Landing this dossier first is also strictly safe in the other direction: reconnect reconciliation is correct at any disconnect frequency. **The reverse order ships a known regression with no repair in place.** |
| Q-1a | Is there a code-level conflict between the two dossiers? | One file, one block. Both edit `useChatroomSocket.ts`: this dossier rewrites the `onStatus` connect burst (`:340-357`), and the socket dossier's F-18 fix touches `resyncPresence` (`:107-115`, specifically the `clearTyping` at `:111`). | Expect a textual conflict in that region if they land close together. Landing this dossier first makes the conflict the socket dossier's to resolve, which is the correct allocation — its change there is a single call, this one restructures the block. |
| Q-2 | On connect, replace `replayDelta` with an invalidation, or fetch the page directly? | **Fetch the page directly on connect and merge it, keeping `applyMessageCreated`'s side-effects for agent rows.** Keep `replayDelta` unchanged for the degraded-mode poll (`:52-55`). | A bare `qc.invalidateQueries` would be one line and would reconcile correctly through `mergeMessages` — but it would **break an existing test**. `useChatroomSocket.test.ts:195-212` ("clears a stale badge when the recovery reply arrives via delta-replay") drives `statusHandlers → true` and asserts the per-agent error badge clears, which happens only inside `applyMessageCreated` (`:145-148`). That test is a deliberate pin, not an accident. Fetching the page directly keeps one request, reconciles deletions, and lets the agent-state side-effects run over the returned rows. |
...
### Q-1 conflict note — read before scheduling either dossier
...
- **Socket-first (that dossier's case).** F-1's churn *generates* the gaps rather than repairing
  ...
  (`useChatroomSocket.ts:347-356`) demonstrably does not reconcile deletions, edits or approvals —
  `replayDelta` appends only and nothing there touches the orchestration store. So removing the
...
**Recommendation for the tie-break:** land **this dossier first**, on risk asymmetry rather than
on exposure arithmetic. Reconnect reconciliation is unconditionally correct and introduces no
regression at any disconnect frequency; the socket fix, landed first, leaves a window in which
long gaps exist with no reconciliation. If the two land in the same release the question is moot,
which is the outcome to prefer. Whichever order is chosen, the Q-1a textual conflict in the
`onStatus` block belongs to whoever merges second.
...
`statusHandlers` (`:17`, registered at `:26-29`) that the existing suite drives in only four places
(`:209`, `:291`, `:312`, `:329`, `:369`, `:403`) and never for a drop-then-restore.

**F-11 — deletion missed during a disconnect.**

```
seedCursor(qc, 'm_old')                 // helper at :102-106
...
hard-deletes and publishes nothing (`retention_service.py:91-93`), so an overnight sweep produces
...
The connect burst (`useChatroomSocket.ts:353-355`) runs three resyncs, none of them approvals; the
card never renders, and the later `approval.resolved` is discarded at `orchestration.ts:32-35`.
...
(`useChatroomSocket.test.ts:49-53`), so `getMessage` is currently `undefined` in that file and the
...
paints "No messages yet — Start the conversation…" (`ChatroomView.vue:97-103`) with the "Load
...
2. The client implements that second half for **creations only**. `replayDelta`
   (`useChatroomSocket.ts:84-105`) calls `listMessages(roomId, {since})` and appends every row via
   `applyMessageCreated` (`:96`, `:135-149`).
...
4. For approvals there is not even a partial read side: no chatroom-scoped list endpoint exists, and
   the `approvals` row carries no room (`tables.py:45-78`), so one cannot be written without a
   schema change.
...
retention purge hard-deletes and publishes nothing (`retention_service.py:91-93`; the module imports
no `Publisher`). So V-2 needs its own fallback regardless, and that fallback is already specified by
the codebase: the `BUG-8` degrade-to-refetch at `useChatroomSocket.ts:97-104`, applied to the
...
`useChatroomSocket.ts:188-201` writes an async result into the cache with no ordering guard, in a
file where `replayGeneration` (`:67,92,95`), `activationGeneration` (`:68,118-127`) and the delete
tombstones (`:76-82`) all exist precisely to prevent this. **This is a missing instance of an
...
pending/error projection (`useChatroomMessages.ts:79-86`, `:327-348`), so the view has nothing to
branch on (`ChatroomView.vue:97-103`). It is in this dossier because it lives in the same two
...
|---|---|
| F-11 | **Yes, the delivery half.** A replayed `message.deleted` / `message.updated` frame would reach the existing handlers (`useChatroomSocket.ts:188-212`) and apply correctly. |
| F-13 | **Yes, the delivery half.** A replayed `approval.requested` would reach `:268-283`. Note this is the *only* remedy that does not need the Q-4 migration — which is a genuine argument for deferring F-13 rather than for skipping it. |
...
- **`useChatroomMessages.confirmDelete` optimistic rollback (`:269-291`) — cleared.** The `catch`
  ...
  class.
- **The degraded-mode poll (`useChatroomSocket.ts:52-55`) — confirmed, same gap, same fix.** It
...
at `useChatroomSocket.ts:353` with a page fetch — `listMessages(roomId, { limit: PAGE_SIZE })` —
...
(`:145-148`) over the returned rows. `replayDelta` stays exactly as it is for the degraded poll
(`:54`) and for `message.created` (`:179`).

Four properties to get right:

- **Carry a generation guard**, reusing `replayGeneration` (`:67,92,95`). A flapping socket
  trivially overlaps two connect fetches.
- **Preserve the agent-badge clear.** `useChatroomSocket.test.ts:195-212` pins that a recovery reply
...
- **Fire on every connect, including the first**, matching the existing block's behaviour
  (`:347-356`) — which also closes the handshake window at no extra cost.
...
`FIX-04` comment (`:176-179`) stating that a delta append replaced a "blind invalidation so the
...
fallback at `useChatroomSocket.ts:97-104` line for line, including its "degrade to a refetch rather
...
ones, captured before `getMessage` and checked on resolve (`:188-201`). Six lines, matching
`:92,95` exactly.

**4. Loading and error states (F-17).** Expose `isPending` and `isError` from `useChatroomMessages`
(`:79-86` → `:327-348`), bind them in `ChatroomView.vue:97-103`: skeleton while pending, error state
...
- Persist it in `ApprovalService.create_gate` (`approval_service.py:72-79`) — the value is already
  in hand at `:69` and already used at `:97`.
...
- Resync it in the connect burst next to the existing three (`useChatroomSocket.ts:353-355`), with
  its own generation guard and its own best-effort catch.
...
`run_engine.py:155` persists the trigger payload so `approval_gate.py:67`'s second arm is
...
tests at `:127-483` must all keep passing, in particular `:195-212`, which constrains the design per
Q-2):

> **`reconciles a message deleted while the socket was down`** — seed the cursor via the existing
> `seedCursor` helper (`:102-106`) with two rows, drive `statusHandlers → false` then `→ true`, have
...
>
> **Why it fails today:** the connect handler calls `replayDelta` (`useChatroomSocket.ts:353`),
> which fetches `{since}` and appends (`:96`). Nothing in that path can remove a cached row — the
> only removal sites are the live `message.deleted` handler (`:202-212`) and `mergeMessages`
...
| `resyncs room approvals on connect` (F-13) | no such API function exists and the connect burst (`:353-355`) makes no approvals call |
| `does not overwrite a newer approval frame with a stale resync response` | mirrors the activation guard test at `:318-343` |
...
| `exposes isPending until the first fetch settles` (F-17) | `:327-348` returns no such flag |
...
`ChatroomView.vue:97-103` and `useChatroomMessages.ts:72`.
...
Extend `backend/tests/unit/test_orchestration_services.py:304-320` (`test_create_gate`, which
already passes `chatroom_id=_ROOM` at `:319`) to assert the value now reaches
`approvals.insert` — today it is used only for the publish at `approval_service.py:97-110`.
...
|---|---|---|---|
| **Layer 1 breaks the pinned badge-clear test** (`useChatroomSocket.test.ts:195-212`) | high if implemented as a bare invalidation | a real behaviour regression, caught in CI | Q-2's decision: fetch the page directly and run the agent-state side-effects over the rows. That test must pass unmodified — if it needs editing, the design is wrong |
| **Reads as a FIX-04 revert** (`useChatroomSocket.ts:176-179`) | medium | a reviewer reverts a correct change | §7 states why `mergeMessages` is the additive merge FIX-04 wanted; the "does not drop older messages" test pins it |
...
| **Overlapping connect fetches on a flapping socket** | medium | stale window applied over fresher data | Generation guard, layer 1. This is exactly why `replayGeneration` exists (`:64-67`) |
...
- [ ] AC-4: `useChatroomSocket.test.ts:195-212` passes **unmodified** — the connect path still clears
...
(`backend/contexts/conversation/application/retention_service.py:91-93`). Correct the comment to
...
- **FU-3** — `useChatroomMessages.refreshOlderMessage` (`:314-325`) has F-19's exact shape with no
  ...
  guard.
- **FU-4** — The degraded-mode poll (`useChatroomSocket.ts:52-55`) calls `replayDelta` and therefore
...
- **FU-5** — The a2u audit's F-15 (the turn watchdog firing on healthy turns) and F-18 (sticking
  ...
  (`findings.md:670,674`). Three dossiers editing one 417-line composable is a coordination hazard
  in its own right; whoever schedules them should consider ordering them strictly rather than in
  parallel.
