...
- [ ] AC-1: The tests from §8.1 fail before the fix and pass after: authoring a type whose key
  matches an opted-in platform type produces a warning, and the type **is still created**.
- [ ] AC-2: Opting into a platform type whose key the project already owns produces the mirror
  warning, and the opt-in **still succeeds**.
- [ ] AC-3: Neither path raises, and no new 409 slug is introduced (§7.1).
- [ ] AC-4: The platform-install path (`example_service.install_course` calling `register` with
  `project_id=None`) is unaffected and produces no warning.
- [ ] AC-5: The facilitator picker and the project type list distinguish a platform row from a
  project row sharing a key and a name, using `scope` rather than a null `project_id`.
- [ ] AC-6: The activity signal carries `activity_type_id` and `activity_type_scope`, both always
  present.
- [ ] AC-7: `matches_activity` with no scope filter behaves identically to today - every existing
  `TestMatchesActivity` case passes unmodified - and a scope-filtered rule matches only the
  intended row.
- [ ] AC-8: `docs/examples/creative-thinking-course.md:312-314` and
  `plugins/mandala9grid/index.ts:1-7` no longer claim keys are unique per project.
- [ ] AC-9: The SRS Delta in §11 is applied to `REQUIREMENTS.md` on approval.
...
- **FU-3**: `uq_activity_types_project_key_active` (`0049_activities.py:78-79`) has no test at
  all, and neither does the `type_repo.py:111-114` arm mapping it to `ActivityTypeKeyConflict` -
  only the platform arm is covered. §8.5 closes it; recorded because the asymmetry is what let the
  cross-scope question go unasked.
...
- **FU-5**: `docs/workflow.schema.json`'s `trigger_config` and `wait_for_event_config` omit
  `additionalProperties: false` (`:179-184`, `:344-350`). This fix depends on that omission being
  intentional. If it is not, adding the constraint later would retroactively invalidate the
  optional fields added here - worth deciding deliberately.
