...
- [x] AC-10: `ruff check .` (825 files clean), `mypy .` (826 files, no issues), and `ruff format --check` for this task's files all pass. Full serial `pytest -m "not integration"` ran to completion: **5676 passed, 54 failed, 6 skipped in 30m21s**. All 54 failures are in `tests/wiring/test_wiring.py` and die at DB/Redis connection setup with `socket.gaierror: [Errno 11001] getaddrinfo failed` — the build environment has no reachable Postgres/Redis (the compose hostnames don't resolve); `tests/wiring/` is de-facto integration-tier but is not tagged with the `integration` marker, so it is not auto-deselected. None of the 54 reach this task's code (they fail before any session opens), and the entire blast radius (orchestration/workflow/a2a/executors: `test_workflow_trigger_loop_guard`, `test_a2a_call_chain`, `test_workflow_signals`, `test_a2a_turn_dispatch`, `test_workflow_executors`, `test_workflow_run_engine`, `test_orchestration_services`, ...) is in the 5676 passing. Zero failures attributable to this change. Note: `ruff format --check .` flags one pre-existing file, `contexts/identity/application/auth_service.py`, owned by the concurrently-in-progress `2026-07-23-google-oauth-login` task — not touched here.
...
- **FU-10** (test infra, pre-existing, out of scope) `tests/wiring/test_wiring.py` needs live Postgres/Redis but carries no `integration` marker, so `pytest -m "not integration"` does not deselect it (the K.7 conftest only auto-marks `tests/integration/`, not `tests/wiring/`). Its 54 tests fail with `getaddrinfo` in any infra-less environment, polluting the fast suite's result. Either move the file under `tests/integration/` or extend the auto-marking conftest to cover `tests/wiring/`.
</content>
