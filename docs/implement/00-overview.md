...
| G | ☑ CODE complete | 2026-04-23 | Migrations 0019–0022 (wakeup_configs, wakeup_authored_snapshots, approvals, instructions, agent_instances). Context: `orchestration` (20 files). Per-agent `a2a:agent:{id}` Redis streams; R9.13 A2A envelope. Wake-up, self-modify, refresh, and agent-only approval (single/majority/consensus per R15.11). Instruct chain: depth=5 limit + loop detection (R15.23). Sub-agent inheritance (R15.22); `key_usage_events.parent_agent_id` carry. `OrchestratorService` + `InstructionService` + `StreamRepository`. Endpoints: `/api/agents/{id}/wake-up`, `/refresh`, `/approve`. Worker task `orchestration.py` (event-driven). Frontend orchestration affordances: ApprovalCard, WakeupConfigEditor, SubagentTree, InstructChainView, DlqViewer in `slices/workflow`. |
| H | ☑ CODE complete | 2026-04-23 | Migrations 0023–0024 (workflows versionless, workflow_runs FSM, workflow_steps). Context: `workflow` (34 files). SEL v1 parser + exact whitelist; 14 linter rules from `workflow.schema.md`; schema validated on save. 11 executors: control-flow, data-transform, approval, agent-invoke, send-message, notification, wait, loop, conditional, assignment, http-request. 5 trigger kinds: cron, manual, change, schedule (no webhook per plan). `workflow_runs` FSM: `running / waiting / succeeded / failed / cancelled`. 90-day archive policy enforced by retention worker. Endpoints: `/api/projects/{}/workflows`, `/api/workflows/{id}`, `/api/workflows/{id}/runs`, `/api/workflows/{id}/steps`. Worker task `workflow.py` (event-driven + cron). Frontend `slices/workflow`: WorkflowListView, WorkflowEditorView, WorkflowBackstageView, WorkflowRunsListView, WorkflowRunView; `useWorkflowRunSocket` real-time step updates; SEL syntax-highlight + validation stub. |
